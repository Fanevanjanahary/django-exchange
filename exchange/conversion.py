...
def convert_values(args_list):
    ...
    """
    rate_map = get_rates(list(map(itemgetter(1, 2), args_list)))
    value_map = {}
...
def get_rates(currencies):
    ...
    if CACHE_ENABLED:
        ...
        for (source, target), rate in list(rate_map.items()):
            if not rate:
                sources.append(source)
                targets.append(target)
    else:
        rate_map = {c: None for c in currencies}
        sources = list(map(itemgetter(0), currencies))
        targets = list(map(itemgetter(1), currencies))
